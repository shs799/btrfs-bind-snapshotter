...
import (
	...
	"btrfs-bind-snapshotter/cmd/btrfs-bind-snapshotter/version"
...
func main() {
	log.L.Infof("containerd-btrfs-bind-grpc Version=%q Revision=%q", version.Version, version.Revision)
...
func serve(address, root string) error {
	...
	sn, err := btrfsbind.NewSnapshotter(root)
	if err != nil {
		return err
	}
