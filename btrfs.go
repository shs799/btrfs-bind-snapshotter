	...
	if err != nil {
		if target != "" {
			if derr := btrfs.SubvolDelete(target); derr != nil {
				if dderr := system.EnsureRemoveAll(target); dderr != nil {
					log.G(ctx).WithError(dderr).WithField("subvolume", target).Error("failed to delete subvolume")
				}
			}
		}
...
func (b *snapshotter) Commit(ctx context.Context, name, key string, opts ...snapshots.Opt) (err error) {
	...
	if source != "" {
		if derr := btrfs.SubvolDelete(source); derr != nil {
			if dderr := system.EnsureRemoveAll(source); dderr != nil {
				// Log as warning, only needed for cleanup, will not cause name collision
				log.G(ctx).WithError(dderr).WithField("subvolume", source).Warn("failed to delete subvolume")
			}
		}
	}
...
func (b *snapshotter) Remove(ctx context.Context, key string) (err error) {
	...
	defer func() {
		if removed != "" {
			if derr := btrfs.SubvolDelete(removed); derr != nil {
				if dderr := system.EnsureRemoveAll(removed); dderr != nil {
					log.G(ctx).WithError(dderr).WithField("subvolume", removed).Warn("failed to delete subvolume")
				}
			}
		}
	}()
	...
	err = b.ms.WithTransaction(ctx, true, func(ctx context.Context) error {
		...
		if err = btrfs.SubvolDelete(source); err != nil {
			if derr := system.EnsureRemoveAll(source); derr != nil {
				return fmt.Errorf("failed to remove snapshot %v: %w", source, derr)
			}
		}
